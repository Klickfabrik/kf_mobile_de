...
class ImporterController extends \TYPO3\CMS\Extbase\Mvc\Controller\ActionController
{
    ...
    private $PATH_site = '';
    private $tmpDir = 'typo3temp/';
    private $tmpImageDir = 'fileadmin/user_upload/_temp_/';
        ...
        if($clientsArray){
            ...
            foreach ($clientsArray as $pos => $client){
                ...
                if(
                    ...
                    if($this->mode == 'single'){
                        $this->createSingleCalls($this->PATH_site . $this->tmpDir . $filename);
                        ...
                        $file = $this->PATH_site . $this->tmpDir . $filename;
                ...
                if(isset($args['test']) && $args['test'] == 1){
                    ...
                    if($this->mode == "single"){
                        ...
                        self::createSingleCalls($this->PATH_site . $this->tmpDir . $filename);
                        foreach ($this->singleList[$filename] as $singleFile){
                            $file = $singleFile['file'];
...
//                        'image_data' => $xml::getTag($imageTag),
                                ...
                                if ($this->isJson($value)) {
                                    ...
                                    foreach ($imageData as $image) {
                                        if (isset($image[$size])) {
                                            ...
                                            $checkFile = $this->PATH_site . "fileadmin/{$target}{$fileName}";

                                            // Download (File fehlt oder Force ist aktiv)
                                            if (!file_exists($checkFile) || $this->overwriteImage == true) {
                                                $curFile = $this->downloadFile($imageURL, $this->PATH_site . $this->tmpImageDir);
                                            } else {
                                                $curFile = str_replace($this->PATH_site . 'fileadmin', '', $checkFile);
                                            }
                    ...
                    if($objNew){
                        ...
                        # Nachträgliches Slugbauen
                        $newElement->setSlug($this->getSlug($newElement));
                        $this->$rep->update($newElement);
                    } else {
                        $this->$rep->update($newElement);
                    }
     ...
     */
    private function getSlug($newElement): string
    {
        return SlugUtility::generateUniqueSlug(
            $newElement->getUid(),
    ...
    private function cleanTypo3temp(){
        ...
        foreach ($filesTypes as $type => $age){
            $files = glob($this->PATH_site . $this->tmpDir . $type);
            ...
            foreach ($repDatas as $repData){
                ...
                if($import && !in_array($id,$data['data'])){
                    if($rep == 'vehicleRepository'){
                        ...
                        // Remove File
                        $target     = $this->PATH_site . 'fileadmin/' . $this->uploadFolder . $data['file'] . DIRECTORY_SEPARATOR;
    ...
    public function uploadTmpFile($filename, $content){
        ...
        $tmpFile = $this->PATH_site . $this->tmpDir . $filename;
    ...
    public function downloadXML($MobileGetter, $client, $single = false){
        ...
        $expireTime = 60 * 15;
        $fileTime   = file_exists( $this->PATH_site . $this->tmpDir . $filename) ? filemtime( $this->PATH_site . $this->tmpDir . $filename ) + $expireTime : 0;
    ...
    private function downloadFile($from, $to)
    {
        ...
        return ($result) ? str_replace($this->PATH_site . 'fileadmin', '',$target) : false;
    }
    ...
    private function createDefaultFolder($subdir= ''){
        $dir = $this->PATH_site . "/fileadmin/{$this->uploadFolder}{$subdir}";
        if(!file_exists($dir)){
            mkdir($dir,0777,true);
        }
    }
